...
class difference_loss(nn.Module):
    ...
    def forward(self, private_samples,shared_samples):
        ...
        private_samples = private_samples - torch.mean(private_samples,0,keepdim=True)#.detach()
        shared_samples = shared_samples - torch.mean(shared_samples,0,keepdim=True)#.detach()

        pn = torch.norm(private_samples,p=2,dim=1, keepdim=True)#.detach()
        sn = torch.norm(shared_samples,p=2,dim=1, keepdim=True)#.detach()
        ...
        # private_samples = torch.div(private_samples , torch.sqrt(torch.sum(private_samples.pow(2),1).view(batch_size,1)))
        # shared_samples = torch.div(shared_samples , torch.sqrt(torch.sum(shared_samples.pow(2),1).view(batch_size,1)))
